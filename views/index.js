...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    // const allData=await axios.get(`http://13.233.174.100:2000/getAll?page=${page}&entries=${entries}`, { headers: { "Authorization": token } })
...
function getAllSelected(page, entries) {
  return new Promise((resolve, reject) => {
    resolve(axios.get(`http://13.233.174.100:2000/getAll?page=${page}&entries=${entries}`, { headers: { "Authorization": token } }))
  }
  )
}
...
function onClick(e) {
  ...
  if (e.target.classList.contains("btn-delete")) {
    ...
    axios.delete(`http://13.233.174.100:2000/deleteExpense/${id}`, { headers: { "Authorization": token } })
      .then(result => {
    ...
    axios.get(`http://13.233.174.100:2000/getDetail/${id}`, { headers: { "Authorization": token } })
      .then(result => {
...
function onSubmit(e) {
  ...
  if (document.getElementById("submitbtn").value == "Update") {
    ...
    axios.patch(`http://13.233.174.100:2000/updateDetails/${id}`, {
      category: category,
    ...
    axios.post("http://13.233.174.100:2000/add-expense", {
      category: category,
      description: description,
      amount: amount
    }, { headers: { "Authorization": token } })
...
function buyPre(e) {
  axios.get("http://13.233.174.100:2000/purchasePremium", { headers: { "Authorization": token } })
    .then((response) => {
      ...
      var options = {
        ...
        "handler": function (result) {
          axios.patch("http://13.233.174.100:2000/purchasePremium/success", {
      ...
      rzpFrontend.on("payment.failed", function (response) {
        ...
        axios.patch("http://13.233.174.100:2000/purchasePremium/failure", {
          order_id: options.order_id,
        }, { headers: { "Authorization": token } })
          .then((result) => {
...
function showLdrBrd() {
  ...
  axios.get("http://13.233.174.100:2000/purchasePremium/showLeaderBoard", { headers: { "Authorization": token } })
    .then(result => {
...
function download() {
  axios.get("http://13.233.174.100:2000/download", { headers: { Authorization: token } })
    .then(result => {
      console.log(result.data);
